...
public class e_929_UniqueEmailAddresses {
    ...
    public int numUniqueEmails(String[] emails) {
        //remove . and anything after + for the local name then add to hashset
        Set<String> unique = new HashSet<>();
        for (String str :
                emails) {
            String[] parts = str.split("@");
            String localName = parts[0].replaceAll("\\.", "").split("\\+")[0];
            if (!unique.contains(localName + "@" + parts[1])) {
                unique.add(localName + "@" + parts[1]);
            }
        }
